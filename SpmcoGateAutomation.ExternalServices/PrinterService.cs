using SpmcoGateAutomation.Common.Configurations;
using SpmcoGateAutomation.Common.Extensions;
using SpmcoGateAutomation.Contract;
using Stimulsoft.Base;
using Stimulsoft.Report;
using System.Drawing.Printing;
using TanvirArjel.Extensions.Microsoft.DependencyInjection;

namespace SpmcoGateAutomation.ExternalServices
{
    [ScopedService]
    public class PrinterService
    {
        private readonly GateConfiguration _gateConfiguration;

        public PrinterService(GateConfiguration gateConfiguration)
        {
            this._gateConfiguration = gateConfiguration;
        }
        public async Task PrintTruckOfConsigneeGateInInfo(ConsigneeGateInPrinterRequestDto printData)
        {
            var reportName = "ConsigneeGateInMonitoringRPT.mrt";
            var printerName = _gateConfiguration.ConsigneeGateInPrinterName;

            if (printData == null)
                return;

            try
            {
                StiLicense.LoadFromString("");
                using (var report = new StiReport())
                {
                    var printPath = @$"{_gateConfiguration.ConsigneeGateReportPath}\{reportName}";
                    report.Load(printPath);
                    report.RegData("Header", printData);
                    report["PrintDate"] = DateTime.Now.ToPersianDateTimeLong();
                    report["PrintOperator"] = $"Console";
                    StiOptions.Print.AllowUsePaperSizesFromPrinterSettings = false;
                    report.Render();
                    PrinterSettings? setting = new() { PrinterName = printerName, Copies = 1 };
                    var ddd = setting.IsValid;
                    report.Print(false, setting);
                }
            }
            catch (Exception ex)
            {
                return ;
            }
        }
        public async Task PrintTruckOfConsigneeGateOutInfo(ConsigneeGateOutPrinterRequestDto printData)
        {
            var reportName = "ConsigneeGateOutMonitoringRPT.mrt";
            var printerName = _gateConfiguration.ConsigneeGateOutPrinterName;

            if (printData == null)
                return;

            try
            {
                StiLicense.LoadFromString("");
                using (var report = new StiReport())
                {
                    var printPath = @$"{_gateConfiguration.ConsigneeGateReportPath}\{reportName}";
                    report.Load(printPath);
                    report.RegData("Header", printData);
                    report["PrintDate"] = DateTime.Now.ToPersianDateTimeLong();
                    report["PrintOperator"] = $"Console";
                    StiOptions.Print.AllowUsePaperSizesFromPrinterSettings = false;
                    report.Render();
                    PrinterSettings? setting = new() { PrinterName = printerName, Copies = 1 };
                   // var ddd = setting.IsValid;
                    if (setting.IsValid)
                    {
                        report.Print(false, setting);
                    }
                }
            }
            catch (Exception ex)
            {
                return;
            }
        }

    }
}
